...
RELAYHOST="${RELAYHOST:-[vnatoday-org.mail.protection.outlook.com]:25}"
...
# SMTP Server Restrictions (Enforce sender validation first)
...
# Escape periods in MYHOSTNAME for the regular expression (e.g., westgate.bank -> westgate\.bank)
ESCAPED_MYHOSTNAME="${MYHOSTNAME//./\\.}"

# Generate sender_access dynamically
printf '/^.+@(.*\\.)?%s$/\tOK\n/./\tREJECT Unauthorized sender domain\n' "$ESCAPED_MYHOSTNAME" > /etc/postfix/sender_access

echo "--> Configuration file [sender_access] created successfully."
